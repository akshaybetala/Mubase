...
TreeNode::TreeNode(char *buffPage) {
	this->buffPage = buffPage;
}
int TreeNode::isLeaf(){
	return readInt(buffPage,SIZE_INT);
}
int TreeNode::getNoPairs(){
	return readInt(buffPage,0);
}
void TreeNode::setIsLeaf(int value){
	writeInt(buffPage,SIZE_INT,value);
}
void TreeNode::setNoPairs(int noPairs){
	writeInt(buffPage,0,noPairs);
}
int TreeNode::getChildPointer(int key){
...
BPlusTree::BPlusTree(const char *dbName, const char *indexName, bool create) {
