...
// Constructor.
SlottedPage::SlottedPage(char *buffPage) {
	this->block = buffPage;
	INTOFFSET=block+4;
}
...
void SlottedPage::initBlock() {
    ...
    writeShort(INTOFFSET, (short)2, (short)(BLOCK_SIZE-4));
}
...
short SlottedPage::storeRecord(char *record , short recLength) {
	...
	if(availableSpace < recLength + 4)
		return (short)-1; 
	eofs= eofs - recLength;
	writeBytes(INTOFFSET,eofs+1,record,recLength);
	writeShort(INTOFFSET,(short)2,eofs);
	
	for(short i=0;i<n;i++)
	{
		short offset=readShort( INTOFFSET,(i+1 )*(short)4 +(short)2 );
		if(offset == -1)
		{
