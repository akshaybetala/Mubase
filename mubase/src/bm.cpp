...
int BufferManager::getTotFreeFrame(){
	int count =0;
	for(int frameId = 0; frameId < NUM_BUFFS; frameId++){
		
		if(blockInFrame[frameId] == -1 || pinCount[frameId] == 0)
			count++;
	}
	return count;
}

char* BufferManager::getTempFrame(char* blockPtr){
	int frameId;
	// If there is enough space in the BufferPool.
	if(filledFrames < NUM_BUFFS){
		frameId = getFreeFrame();	
		assert(frameId != -1);
		memcpy(buffer + frameId * BLOCK_SIZE,blockPtr,BLOCK_SIZE);
