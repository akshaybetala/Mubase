...
Sort::Sort(char* dbName,char* relName, int attrIndex,bool order){
	this->dbName=dbName;
...
void Sort::open(){
	...
	RelBlockIterator relBlockIterator(dbName,relName);
	relBlockIterator.open();
	int CurrentRunNo=0;
	int TotalRun=0;
	bool rel=true;
	while(rel){
		int FreeFrames=bm->getTotFreeFrame(true)-1;
		...
		for(int i =0;i < FreeFrames;i++){
			...
			blockPtr[i] = bm->getTempFrame( bm->pinPage(blockId,true), true );
		...
		CurrentRunNo++;
		TotalRun++;
		string runName= "run" + (char)(CurrentRunNo);
...
bool Sort::recCompare(char* rec1, char* rec2){
	...
	if(this->sortOrder) return !( record1.greaterThan(record1,attrIndex) );
	else return !( record1.lessThan(record1,attrIndex) );
}


bool Sort::RecElement::operator<(const RecElement& b){
   return (this->rec)->lessThan(*(b.rec),this->parent->attrIndex);
}

bool Sort::RecElement::operator>(const RecElement& b){
	return (this->rec)->greaterThan(*(b.rec),this->parent->attrIndex);
}

bool Sort::RecElement::operator==(const RecElement& b){
	return (this->rec)->equalTo( *(b.rec),this->parent->attrIndex);
}
